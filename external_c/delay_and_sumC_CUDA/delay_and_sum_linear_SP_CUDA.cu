...
//=========================================================================================================
//=========================================================================================================
// (GPU Cuda Kernel) Single Precision Delay and Sum Algorithm, using linear interpolation 
//=========================================================================================================
//=========================================================================================================
__global__ void c_delay_and_sum_amplitudes_linear_Kernel_SP( const float* scanlines,  const unsigned int* tx,  const unsigned int* rx,  const float* lookup_times_tx,  const float* lookup_times_rx,
                ...
                int numpoints,int numsamples,int numelements, int numscanlines)
{
    ...
    if (point < numpoints)
    {
        ...
        float tot_real1,tot_imag1,amp_corr,lookup_time,frac1;
        tot_real1=0.0;
        tot_imag1=0.0;
...
//=========================================================================================================
//=========================================================================================================
// SINGLE PRECISION EXTERNAL C CODE (LINEAR INTERPOLATION)
//=========================================================================================================
//=========================================================================================================
int c_delayAndSum_Algorithm_linear_SP_CUDA(
...

{
    ...
    {
        // Input
        cudaMalloc(&scanlines_gpu, Nscan*2*sizeof(float)); 
        cudaMalloc(&tx_gpu, numscanlines*sizeof(unsigned int)); 
        cudaMalloc(&rx_gpu, numscanlines*sizeof(unsigned int)); 
        cudaMalloc(&lookup_times_tx_gpu, Nlook*sizeof(float));
        cudaMalloc(&lookup_times_rx_gpu, Nlook*sizeof(float));
        cudaMalloc(&amplitudes_tx_gpu, Nlook*sizeof(float));
        cudaMalloc(&amplitudes_rx_gpu, Nlook*sizeof(float));
        // Output
        cudaMalloc(&result_gpu, numpoints*2*sizeof(float));
    ...
    cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, c_delay_and_sum_amplitudes_linear_Kernel_SP, 0, numpoints); 
    // Round up according to array size 
    gridSize = (numpoints + blockSize - 1) / blockSize; 

    // Perform Kernel Operations
    //=========================================================================
    c_delay_and_sum_amplitudes_linear_Kernel_SP<<<gridSize, blockSize>>>(scanlines_gpu,tx_gpu,rx_gpu,lookup_times_tx_gpu,lookup_times_rx_gpu,
